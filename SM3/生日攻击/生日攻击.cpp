...
std::string uniqueName(int length) {
	auto randchar = []() -> char
	{
...
int compare(string str[][2], int size,int len)
{
	string temp = str[size][1];
	for (int i = size - 1; i > -1; i--)
	{
		if (str[i][1].substr(0, len) == str[size][1].substr(0, len)&& str[i][0]!=str[i][0])
		{
			cout << "Msg1:"<<str[size][0] << endl <<"Msg2:"<< str[i][0] << endl;
			cout << "Hashed_Msg1：" << endl;
			for (int j = 0; j < 8; j++)
			{
				cout << str[size][1].substr(j * 8, 8) << "  ";
			}
			cout << endl << "Hashed_Msg2：" << endl;
			for (int j = 0; j < 8; j++)
			{
				cout << str[i][1].substr(j * 8, 8) << "  ";
			}
			return i;
		}
	}
...
#define MAX 10000
	...
	for (int i=0;i<MAX;i++)
	{
		str[i][0] = uniqueName(10);
		string paddingValue1 = padding(str[i][0]);
		str[i][1]= iteration(paddingValue1, IV);
		if (compare(str, i, 4)!=-1)
			break;
	}
	return 0;
}